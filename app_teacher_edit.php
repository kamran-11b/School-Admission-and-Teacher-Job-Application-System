...
if (isset($_SESSION["admin_email"]) || isset($_COOKIE['admin_email'])) {
    ...
    $up_id=$_GET['applicant_id'];

    $fq = "SELECT * FROM `applicant_teacher` WHERE applicant_id='$up_id' ";
    ...
    if(isset($_POST['update'])){
        ...
        $update_query=" UPDATE `applicant_teacher` SET applicant_id='$applicant_id',payment_status ='$payment_status' WHERE  applicant_id='$applicant_id' ";
                    ...
                    while ($row = $result->fetch_assoc()){?>
                        <form action="<?php $_SERVER["PHP_SELF"]; ?>" method="POST" class="jumbotron" > 
                            ...
                            <div class="form-group row ">
                                <label for="applicant_id" class="col-sm-3  control-label">Applicant ID</label>
                                <div class="col-sm-5">
                                    <input type="text" class="form-control form-group" name="applicant_id"  value="<?php echo $row['applicant_id'] ?>" autofocus required>
                                </div>
                            </div>
